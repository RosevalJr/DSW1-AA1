...
public class EmpresasUserController extends HttpServlet {
	...
	private void avaliaCandidatura(HttpServletRequest request, HttpServletResponse response)
	...
	private void listaCandidaturas(HttpServletRequest request, HttpServletResponse response)
			throws ServletException, IOException {
		Long idvaga = Long.parseLong(request.getParameter("idvaga"));
	...
	private void apresentaFormAvaliacao(HttpServletRequest request, HttpServletResponse response)
			throws ServletException, IOException {
		Long idvaga = Long.parseLong(request.getParameter("idvaga"));
		...
		RequestDispatcher dispatcher = request
				.getRequestDispatcher("/logado/user/empresa/listaAvaliacaoCandidatura.jsp");
		dispatcher.forward(request, response);
	}
	 ...
	 * throws IOException { Long id = Long.parseLong(request.getParameter("id"));
	 * 
	 * Empresa empresa = new Empresa(id); dao.delete(empresa);
	 * response.sendRedirect("lista"); }
	 * 
	 * private void apresentaFormEdicao(HttpServletRequest request,
	 * HttpServletResponse response) throws ServletException, IOException { Long id
	 * = Long.parseLong(request.getParameter("id")); Empresa empresa = dao.get(id);
	 * RequestDispatcher dispatcher =
	 * request.getRequestDispatcher("/logado/admin/empresa/formulario.jsp");
	 * request.setAttribute("empresa", empresa); dispatcher.forward(request,
	 * response); }
	 * 
	 * private void atualize(HttpServletRequest request, HttpServletResponse
	 * response) throws ServletException, IOException {
	 * 
	 * request.setCharacterEncoding("UTF-8"); Long id =
	 * Long.parseLong(request.getParameter("id")); Long cnpj;
	 * 
	 * try { cnpj = Long.parseLong(request.getParameter("cnpj")); } catch
	 * (NumberFormatException nfe) { Erro erros = new Erro();
	 * erros.add("O campo cnpj deve ser apenas comoposto por números");
	 * request.setAttribute("mensagens", erros); RequestDispatcher rd =
	 * request.getRequestDispatcher("/erros.jsp"); rd.forward(request, response);
	 * return; } String nome = request.getParameter("nome"); String senha =
	 * request.getParameter("senha"); String email = request.getParameter("email");
	 * String descricao = request.getParameter("descricao"); String cidade =
	 * request.getParameter("cidade");
	 * 
	 * Empresa empresa= new Empresa(id, cnpj, nome, senha, email, descricao,
	 * cidade);
	 * 
	 * dao.update(empresa); response.sendRedirect("lista"); }
	 */
